...
const Navbar = () => {
  ...
  return (
    <div className='shadow-md w-full fixed top-0 left-0 border-white border-y-2'>
      ...
      <ul className={`font-medium  md:flex md:items-center md:pb-0 pb-12 absolute md:static bg-black text-white md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9  duration-500 ease-in ${open ? 'top-20 ':'top-[-490px]'}`}>
        {
          Links.map((link)=>(
            <li key={link.name} className='md:ml-8 text-xl md:my-0 my-7'>
              <a href={link.link} className='text-white hover:text-yellow-600 duration-500'>{link.name}</a>
            </li>
          ))
        }
        <Button>
